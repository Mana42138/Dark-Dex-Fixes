...
local function main()
	...
	Explorer.BuildSearchFunc = function(query)
		...
		local template = [==[
local searchResults = _G.searchResults
local nodes = _G.nodes
local expandTable = _G.Explorer.SearchExpanded
local specResults = _G.specResults
local service = _G.service
...
return search]==]
        ...
        --local env = setmetatable({["searchResults"] = searchResults, ["nodes"] = nodes, ["Explorer"] = Explorer, ["specResults"] = specResults,["service"] = service},{__index = getfenv()})
        _G.searchResults = searchResults
        _G.nodes = nodes
        _G.Explorer = Explorer
        _G.specResults = specResults
        _G.service = service
	...
	Explorer.Init = function()
		...
		local insts = game:GetDescendants()
		if Main.Elevated then
			for i = 1,#insts do
				local obj = insts[i]
				local par = nodes[ffa(obj,"Instance")]
...
Main = (function()
	...
	Main.Init = function()
		...
		if Main.Elevated then
			...
			Main.RobloxVersion = "0.624.0.6240521"
		end
